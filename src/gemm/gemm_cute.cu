...
#pragma unroll
    ...
    // Explicitly convert F32 accumulator to output type T (BF16) before store.
    // retile_S on tCrD directly would reinterpret F32 bits as BF16 pairs (wrong).
    ...
    auto tCrC_r2s = r2s_thr_copy_c.retile_S(tCrD_out);  // (CPY, CPY_M, CPY_N)
...
namespace config
{
    ...
    struct GemmConfig
    {
        ...
        using SmemLayoutAtomC = decltype(composition(
            Swizzle<2, 3, 3>{}, make_layout(make_shape(Int<kMmaPM>{}, Int<kMmaPN>{}), make_stride(Int<kMmaPN>{}, Int<1>{}))));
...
// Host launcher: A (M,K) row-major, B_T (N,K) row-major (already transposed), D (M,N) row-major
// Computes D = A * B^T. M must be divisible by 128, N by 128, K by 32.
void gemm_cute(void *Dptr, const void *Aptr, const void *Bptr,
               int M, int N, int K, cudaStream_t stream)
{
    using Cfg = config::GemmConfig<cutlass::bfloat16_t>;

    auto kernel = gemm_multi_stage<Cfg>;
    int smem_size = static_cast<int>(Cfg::kShmSize);
    ...
    kernel<<<grid, block, smem_size, stream>>>(Dptr, Aptr, Bptr, M, N, K);
}
